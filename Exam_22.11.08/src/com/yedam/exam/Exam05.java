		...
		while(run) {
			...
			if(selectNo == 1) {		//주사위 크기 입력		
				...
				while(true) {	// index = num-1 , index+1 = num
					...
					count++;
					dice[num-1] = dice[num-1]+1;	//배열이 만들어지기만 했지 배열 내용은 빈칸이기 때문에 나오는걸 채워줘야한다.
													//num은 1~6(=사용자 지정숫자)까지의 수이기 때문에 인덱스 숫자보다 +1큼. 그래서 인덱스 번호를 나타내려면 -1해줘야됨
													//ex)굴려서 4나옴. dice[3] = dice[3]+1  p.183 => 위에서 new int로 선언해줬으니까 배열 길이만 생성되어있고
													//배열 값은 다 0으로 채워져있다. 그래서 0대신에 나온숫자자리에 +1를 해줘서 나왔다 안나왔다 표시를 해주는것.
													//(좌)그 부여받은 인덱스 번호에다가 (우)0대신 1을 적어서 표시
